...
Instructions for coding agents in this repository. `CLAUDE.md` imports this file,
so every agent reads the same thing.

## Conventions

- **Commits**: [Conventional Commits](https://www.conventionalcommits.org/).
- **PRs**: squash merge only. The squash commit takes the PR title and
  description, so write them as the permanent commit message.
- **Copyright headers**: added by the `addlicense` pre-commit hook. Never write
  them by hand.
- **Python**: managed with **uv**. Never pip, never Poetry, and never
  `pip install --break-system-packages`.
- **Pre-commit**: `make pre-commit-install` once per clone; `make pre-commit` to
  run every hook. `make` lists all targets.
...
This project uses [pre-commit](https://pre-commit.com/) to enforce copyright headers,
formatting, secret scanning, and commit messages. Install once after cloning:

```bash
make pre-commit-install    # creates a gitignored .venv via uv, installs the git hooks
```

Hooks run automatically on `git commit`. To run manually against every file:
...
- **addlicense** — stamps `Copyright <year> The Financial Risk Group, Inc.` headers.
  Never write them by hand. Skips `src/_js_parser/` and `CHANGELOG.md`.
- **prettier** — formats `.ts`, `.js`, `.json`, `.md`, `.yaml` files
- **markdownlint-cli2** — lints Markdown against `.markdownlint.json`. `MD007.indent` is 2 rather than FRG's
  usual 4, because prettier writes 2-space list indents and prettier is the formatter of record here.
- **gitleaks** / **detect-private-key** — secret scanning
- **check-json**, **check-yaml**, **check-toml**, **check-merge-conflict**,
  **check-added-large-files** — file sanity checks
...
- **conventional-pre-commit** (`commit-msg` stage) — enforces conventional commit format locally.
  This is the only automated commit-message check; there is no CI equivalent, because squash
  merges replace commit messages with the PR title and body (allowed types: feat, fix, docs,
  style, refactor, test, build, ci, chore, revert, perf). Nothing validates the PR title, which
  is the message that lands on `main` and the one release-please reads to pick the version bump —
  getting that right is on the author and the reviewer.
...
- **Auto-completion**: Context-aware suggestions scoped by statement type (`in`, `out`, `node`, `model`,
  top-level); CSV file names sourced from `CsvFileCache`
- **Hover documentation**: `KEYWORD_DOCS` map in `hoverProvider.ts` covers all top-level keywords plus
  node/model/subprocess signatures
...
The extension activates when a `.strm` file is opened (`onLanguage:strm`). All providers and commands are
registered during activation.

### Test Infrastructure

Tests use Mocha 11 + `@vscode/test-electron`. Test files live in `src/test/suite/` and cover the parser,
completion provider, document symbol provider, and hover provider.
...
**Each January:** manually set `.release-please-manifest.json` to `"27.0.0"` (or the current year) and commit
to `main` before the first release of that year.
...
5. **Marketplace publish is manual** — download the `.vsix` from the GitHub Release and run `vsce publish` if
