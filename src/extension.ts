        ...
        'strm',
        {
            async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                const predefinedCompletions = [
                    ...
                    { label: `NAME ${fileName}`, insertText: `NAME ${fileName}`, detail: "Suggests filename", kind: vscode.CompletionItemKind.Variable },
                    // 🔹 TYPE= Completion: Suggest MODEL, REPORT, DEFAULT
                    { label: 'TYPE', insertText: 'TYPE ${1|DEFAULT,REPORT,MODEL|}', detail: "Select a type", kind: vscode.CompletionItemKind.Enum },
                    // 🔹 LANG= Completion: Suggest PYTHON, GO
                    { label: 'LANG', insertText: 'LANG=${1|GO,PYTHON|}', detail: "Select a language", kind: vscode.CompletionItemKind.Enum },
                    // 🔹 DB= Completion: Suggest PG, MSSQL
                    { label: 'DB', insertText: 'DB=${1|PG,MSSQL|}', detail: "Select a database", kind: vscode.CompletionItemKind.Enum },
                    // 🔹 MODE= Completion: Suggest APPEND, REPLACE
                    { label: 'MODE', insertText: 'MODE=${1|APPEND,REPLACE|}', detail: "Select a mode", kind: vscode.CompletionItemKind.Enum }
                ];
                ...
                const parentDirectory = path.join(path.dirname(path.dirname(document.fileName)), 'input');
                const directoriesToCheck = [inputDirectory, parentDirectory];
                const csvFiles: string[] = await getCSVFiles(directoriesToCheck);
                const csvFilesString = csvFiles.length > 0 ? csvFiles.join(',') : "first.csv";

                const snippetTemplates = [
                    { type: "INPUT", label: "CSV", snippet: `INPUT \${1|${csvFilesString}|} -> \${2:input_queue}`, description: `CSV Input Template
> INPUT \${1|${csvFilesString}|} -> \${2:input_queue}
                                        ` },
                    { type: "INPUT", label: "DB", snippet: "INPUT ${1:schema_name}.${2:table_name} -> ${3:input_queue}", description: descriptions['INPUT_DB'] },
                    { type: "INPUT", label: "S3", snippet: "INPUT s3://${1:bucket_name}/${2:path} -> ${3:input_queue}", description: descriptions['INPUT_S3'] },
                    { type: "OUTPUT", label: "CSV", snippet: "OUTPUT ${1:output_queue} -> ${2:output.csv}", description: descriptions['OUTPUT_CSV'] },
                    { type: "OUTPUT", label: "DB", snippet: "OUTPUT ${1:output_queue} -> ${2:schema_name}.${3:table_name}", description: descriptions['OUTPUT_DB'] },
                    { type: "OUTPUT", label: "S3", snippet: "OUTPUT ${1:output_queue} -> s3://${2:bucket_name}/${3:path}", description: descriptions['OUTPUT_S3'] }
                ];
                ...
                const nodeItem = new vscode.CompletionItem('NODE', vscode.CompletionItemKind.Keyword);
    ...
    const openInputFileCommand = vscode.commands.registerCommand("extension.openInputFile", async () => {
    ...
    if (!match || position.character <= match.index! + 4) { 
        vscode.window.showErrorMessage("No INPUT statement found at the cursor.");
        return;
    }
    ...
    vscode.window.showErrorMessage(`File not found in expected location:\n- ${filePath}`);
    });

    context.subscriptions.push(openInputFileCommand);


    const openNodeFileCommand = vscode.commands.registerCommand("extension.openNodeFile", async () => {
        const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
    ...
    if (!match || position.character <= match.index! + 4) { 
        vscode.window.showErrorMessage("No NODE statement found at the cursor.");
        return;
    }
    ...
    const possiblePaths = [
        path.join(workspaceFolder, 'src', 'nodes', `${nodeName}`, `${nodeName}U.go`), // Workspace root/src
    ...
    vscode.window.showErrorMessage(`File not found in expected locations:\n- ${possiblePaths.join("\n- ")}`);
    });
    ...
    let disposable = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            const terminal = vscode.window.terminals.find(t => t.name === "VOR Terminal") 
...
function insertTemplate(template: string) {
    ...
    editor.edit((editBuilder) => {
        ...
        const message = success ? "Inserted .strm template." : "Failed to insert template.";
        success ? vscode.window.showInformationMessage(message) : vscode.window.showErrorMessage(message);
    });
}

// 🔹 Function to create a completion item
function createCompletionItem(label: string, insertText: string, detail: string, kind: vscode.CompletionItemKind): vscode.CompletionItem {
...
function createSnippet(type: string, label: string, snippet: string, description: string): vscode.CompletionItem {
...
function registerCommands(context: vscode.ExtensionContext) {
    Object.entries(templates).forEach(([lang, template]) => {
        ...
        const command = vscode.commands.registerCommand(commandId, () => insertTemplate(template));
